...
workflows: Dict[e.WorkflowInstanceId, e.Workflow] = {}
...
async def start_workflow(
    ...
    workflows[workflow.iid] = workflow
    return workflow
...
async def complete_workflow(
    ...
    del workflows[instance_id]


@validate_arguments
async def start_tasks(tasks: List[e.TaskStartInput]) -> None:
    created_tasks = []
    for task in tasks:
        new_task = e.Task(
            id=task.id,
            data=[e.TaskData(**d.dict()) for d in task.data],
        )
...
async def _save_tasks(
    ...
    bridge = REXFlowBridge(workflows[iid])
    return await bridge.save_task_data(tasks)


@validate_arguments
async def save_tasks(tasks: List[e.Task]) -> List[e.Task]:
