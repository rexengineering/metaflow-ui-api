...
mutation ValidateTaskData($validateTaskDataInput: ValidateTaskExchangeInput!) {
    workflow {
        tasksExchange {
            validate(
                input: $validateTaskDataInput
...
mutation SaveTaskData($saveTaskDataInput: SaveTasksExchangeInput!) {
    workflow {
        tasksExchange {
            save(
                input: $saveTaskDataInput
...
COMPLETE_TASK_EXCHANGE_DATA_MUTATION = '''
mutation CompleteTaskData($completeTasksInput: CompleteTasksExchangeInput!) {
    workflow {
        tasksExchange {
            complete(
                input: $completeTasksInput
...
class IntegrationTestAmortization(unittest.TestCase):
    ...
    @run_async
    async def test_amortization_workflow_with_xid(self):
        # List available workflows
        available_workflow_query = gql(AVAILABLE_WORKFLOW_QUERY)
        async with self.client as session:
            await session.execute(available_workflow_query)
        ...
        while len(tasks) == 0:
            ...
            get_task_data_query = gql(GET_TASK_EXCHANGE_DATA_QUERY)
            get_task_data_params = {
                'workflowFilter': {'ids': [workflow_iid]}
            }
        ...
        self.assertGreater(count, 0)
        self.assertGreater(len(tasks), 0, f'workflow id {workflow_iid}')
        task_tid = tasks[0]['tid']
        task_xid = tasks[0]['xid']
        self.assertEqual('get_terms', task_tid, f'workflow id {workflow_iid}')
        ...
        validate_task_data_mutation = gql(VALIDATE_TASK_EXCHANGE_DATA_MUTATION)
        validate_task_data_params = {
            'validateTaskDataInput': {
                'tasks': [task_data]
            },
        ...
        status = result['workflow']['tasksExchange']['validate']['status']
        self.assertEqual(
            status,
            OperationStatus.SUCCESS,
            f'workflow id {workflow_iid}',
        )
        task = result['workflow']['tasksExchange']['validate']['tasks'][0]
        for data in task['data']:
            self.assertEqual(
                data['data'],
                data_values[data['dataId']],
        ...
        save_task_data_mutation = gql(SAVE_TASK_EXCHANGE_DATA_MUTATION)
        save_task_data_params = {
            'saveTaskDataInput': {
                'tasks': [task_data]
            },
        ...
        status = result['workflow']['tasksExchange']['save']['status']
        self.assertEqual(
            status,
            OperationStatus.SUCCESS,
            f'workflow id {workflow_iid}',
        )
        task = result['workflow']['tasksExchange']['save']['tasks'][0]
        for data in task['data']:
            self.assertEqual(
                data['data'],
                data_values[data['dataId']],
        ...
        complete_task_data_mutation = gql(COMPLETE_TASK_EXCHANGE_DATA_MUTATION)
        complete_task_data_params = {
            'completeTasksInput': {
                'tasks': [task_data]
            },
        ...
        status = result['workflow']['tasksExchange']['complete']['status']
        self.assertEqual(
            status,
            OperationStatus.SUCCESS,
            f'workflow id {workflow_iid}',
        )
        task = result['workflow']['tasksExchange']['complete']['tasks'][0]
        for data in task['data']:
            self.assertEqual(
                data['data'],
                data_values[data['dataId']],
        ...
        task_tid = tasks[0]['tid']
        task_xid = tasks[0]['xid']
        self.assertEqual('show_table', task_tid, f'workflow id {workflow_iid}')
